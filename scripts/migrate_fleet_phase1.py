...
from app.services.fleet_migration import apply_fleet_phase1, rollback_fleet_phase1  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser(description="Aplica ou reverte a migration da Frota Fase 1 com snapshot local.")
...
def main() -> int:
    ...
    if args.action == "apply":
        ...
        if args.dry_run:
            raise ValueError("--dry-run só é aceito com a ação apply.")
        report = rollback_fleet_phase1(
            data_dir=data_dir,
            backups_dir=backups_dir,
